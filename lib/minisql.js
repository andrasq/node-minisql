...
function Packman( ) {
    ...
    // or use the renewed legacy packeter
    this.packeter = new Packeter()
    // use the renewed chunker
...
function Packeteer( options ) {
    ...
    this.get = function get( ) {
        ...
        var need = this.needed ? this.needed : this.needed = options.bytesNeeded(this.bufs)
        ...
        return options.assemblePayload(chunks)
    }
}
// combine network data chunks into mysql packets
function Chunker( ) {
    this.bufs = new Array()
    this.nbytes = 0
    this.needed = 0
...
// combine mysqls packets into messages
function Packeter( ) {
    this.frags = []
    this.packets = []
    this.write = function write(chunk) {
