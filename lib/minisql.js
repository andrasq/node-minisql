...
function Db( options ) {
    ...
    return linkCount === 1 ? this.sessions[0].sessionDb : this
}
...
Db.prototype.connect = function connect( callback ) {
    ...
    utils.repeatFor(self.sessions.length, function(next, ix) {
        self.sessions[ix]._connect(creds, function(err) {
...
Db.prototype.end = function end( callback ) {
    var session, self = this, nleft = this.sessions.length, errors = []
    for (var i=0; i<this.sessions.length; i++) {
        (session = this.sessions[i]).runQueries(self.options.teardown, function(err) {
...
Session.prototype._connect = function connect( creds, callback ) {
    ...
    this.packman.getPacket(function(never, buf) {
        ...
        if (packet._seqId !== 0) return callback(_makeError('handhake got wrong sequence num %d, expected 0', packet._seqId));
    ...
    (function _loop() {
        self.packman.getPacket(function(never, data) {
            if (data[3] < nextSeqId++) {
                ...
                var err = _makeError('got wrong sequence num %d, expected %d', data[3], nextSeqId - 1)
                self.fatal(err); return callback(err, packets, nextSeqId)
            }
...
Session.prototype._readResult = function _readResult(query, nextSeqId, t1, callback) {
    ...
    this.packman.getPacket(function(never, buf) {
        ...
        if (buf[3] < nextSeqId++) return _done(_makeError('got wrong sequence num %d, expected %d', buf[3], nextSeqId - 1))
...
Session.prototype.fatal = function fatal( err ) {
