...
function Db( options ) {
    ...
    // optimize single-connection case, talk to the link
    return this.options.sessions > 1 ? this : this.getConnection()
}
...
Db.prototype.getConnection = function getConnection( ) {
    ...
    var conn = this.sessions.shift()
    this.sessions.push(conn)
    return conn
}
