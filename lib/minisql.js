...
function Packeteer( options ) {
    ...
    this.bufs = []
    this.nbytes = 0
    this.minBytes = options.minBytes || 4 // allow for the zero-length packet following len 0xffffff
    this.bytesNeeded = options.bytesNeeded || function(bufs) { return 4 + my.readUint3(bufs[0], 0) }
    this.finalLength = options.finalLength || function(bufs) { return 4 + my.readUint3(bufs[0], 0) }
    ...
    this.get = function get( ) {
        ...
        var msg = utils.allocBuf(this.finalLength(this.bufs))
        ...
        for (var ix = 0, n = 0, have = 0; need > 0; ++ix, need -= n, have += n) {
            if (need >= this.bufs[0].length) { n = this.extractPayload(ix, this.bufs.shift()).copy(msg, have) }
            else { n = this.extractPayload(ix, this.bufs[0]).copy(msg, have, 0, need); this.bufs[0] = this.bufs[0].slice(need) }
        }
        return msg
    }
}
