...
function Packman( ) {
    ...
    this.error = null
    this.chunker = new Packeteer({      // data chunks -> mysql packets
        id: 'chunker',
        minBytes: 4, // allow for the zero-length packet following len 0xffffff
        bytesNeeded: function(bufs) { return 4 + my.readUint3(bufs[0], 0) },
        finalLength: function(needed, bufs) { return needed },
        extractPayload: null
    })
    this.packeter = new Packeteer({     // mysql packets -> mysql responses
        id: 'packeter', minBytes: 4,
        bytesNeeded: function(bufs) {
            var tot = 0, len, i = 0
            do { tot += 4 + (len = my.readUint3(bufs[i++], 0)) } while (len >= 0xffffff && i < bufs.length)
//console.log("AR: bytesNeeded(%d) returning: %d", bufs.length, tot, bufs)
            return len < 0xffffff ? tot : 0
        },
        finalLength: function(needed, bufs) { return needed - 4 * (bufs.length - 1) },
        extractPayload: function(ix, needed, buf) { return ix > 0 ? buf.slice(4) : buf }
    })
    this._socket = null
}
...
/** FIXME: buggy, breaks tests
        self.chunker.writeChunk(chunk)
        var msgbuf, response
        while ((msgbuf = self.chunker.get())) {
            self.packeter.writeChunk(msgbuf)
...
//console.log("AR: getPacket got", packet && packet.length, this.waitlist.length)
...
/*
 * Data chunker and recombiner.
 * mysql wire protocol packet concatenator: write() chunks and get() full packets
 ...
 */
function Packeteer( options ) {
    options = options || {}
    this.id = options.id
    this.bufs = []
    this.nbytes = 0
    this.minBytes = options.minBytes || 4
    this.bytesNeeded = options.bytesNeeded || function(bufs) { return bufs[0].length }
    this.finalLength = options.finalLength || function(needed, bufs) { return bufs[0].length }
    this.extractPayload = options.extractPayload || null
    this.needed = 0

    this.writeChunk = function writeChunk( chunk ) {
        // ensure that the first chunk contains the length + seq id + header bytes: 3b + 1b + 1b
        var buf = this.bufs[0]
        if (buf && buf.length < this.minBytes) this.bufs[0] = Buffer.concat([buf, chunk])
        else this.bufs.push(chunk)
        this.nbytes += chunk.length
    }
    this.get = function get( ) {
//console.log("AR: %s packeter GET", this.id, this.needed)
        if (this.nbytes < this.minBytes) return null
        var need = this.needed || (this.needed = this.bytesNeeded(this.bufs))
//console.log("AR: %s packeteer needs", this.id, need)
        if (need <= 0) { this.needed = 0; return null }
        if (this.nbytes < need) return null
//console.log("AR: %s packeter getting", this.id, need, this.needed, this.nbytes, this.bytesNeeded(this.bufs))

//console.log("AR: %s packeter getting; need", this.id, need, this.nbytes)

        if (this.bufs[0].length >= need) { // common case, the small control packets
            var buf = this.bufs[0]; this.nbytes -= need; this.needed = 0
            var retbuf = (buf.length === need) ? this.bufs.shift() : ((this.bufs[0] = buf.slice(need)), buf.slice(0, need))
            return this.extractPayload ? this.extractPayload(0, need, retbuf) : retbuf
        }

//console.log("AR: %s packeteer assembling packets to total %d bytes", this.id, need, this.nbytes, this.bufs.length)
        var finalLength = this.finalLength(need, this.bufs), msg = utils.allocBuf(finalLength)
        this.nbytes -= need; this.needed = 0
        // combining chunks here adds 3.2ms per 17mb copied
        for (var ix = 0, n = 0, have = 0; have < finalLength; ++ix, need -= n, have += n) {
//console.log("AR: %s: copying #%d buf", this.id, ix, need)
            if (need >= this.bufs[0].length) { var retbuf = this.bufs.shift() }
            else { var retbuf = this.bufs[0].slice(0, need); this.bufs[0] = this.bufs[0].slice(need) }
            n = this.extractPayload ? this.extractPayload(ix, need, retbuf).copy(msg, have) : retbuf.copy(msg, have)
        }
//console.log("AR: %s packeteer built %d bytes", this.id, msg.length)
        return msg
    }
}
