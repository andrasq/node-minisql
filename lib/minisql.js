...
function Packeteer( options ) {
    ...
    this.finalLength = options.finalLength || this.bytesNeeded
    this.extractPayload = options.extractPayload || function(ix, buf) { return buf }
    this.needed = 0
    ...
    this.get = function get( ) {
        ...
        if (this.bufs[0].length >= need) { // common case, the small control packets
            ...
            return (buf.length === need) ? this.bufs.shift() : ((this.bufs[0] = buf.slice(need)), buf.slice(0, need))
        }
        ...
        for (var ix = 0, n = 0, have = 0; need > 0; ++ix, need -= n, have += n) {
            if (need >= this.bufs[0].length) {
                n = this.extractPayload(ix, this.bufs.shift()).copy(msg, have)
            } else {
                n = this.extractPayload(ix, this.bufs[0].slice(0, need)).copy(msg, have, 0, need);
                this.bufs[0] = this.bufs[0].slice(need) }
        }
        return msg
    }
}
