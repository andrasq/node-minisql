...
function Db( options ) {
    ...
    var self = this
    this.sessions = new Array(linkCount)
    this.states = new Array(linkCount)
    this.lastSession = 0
    for (var i = 0; i < linkCount; i++) (function(i) {
        var session = new Session(self.options)
        session.connect = function(cb) { return self.connect(cb) }
        session.end = function(cb) { return self.end(cb) }
        session.notifyBusy = function() { self.states[i] = 1 }
        session.notifyIdle = function() { self.states[i] = 0 }
        session._getRawDb = function() { return self }
        self.sessions[i] = session
        self.states[i] = 0
    })(i)

    // optimize, in single-connection case talk to the connection directly
    return linkCount === 1 ? this.getConnection() : this
}
Db.prototype.getConnection = function getConnection( ) {
    return this._getConnection()
}
Db.prototype._getAllConnections = function _getAllConnections() { return this.sessions }
Db.prototype._getConnection = function _getConnection( ) {
    // simple round-robin lru
    var i, sessions = this.sessions

    for (i = 0; i < sessions.length; i++) if (this.states[i] === 0) return (this.lastSession = i, sessions[i])
    this.lastSession = (this.lastSession + 1) % sessions.length
...
Db.prototype.connect = function connect( callback ) {
    ...
    var sessions = this._getAllConnections()
...
Db.prototype.end = function end( callback ) {
    ...
    for (var i=0; i<sessions.length; i++) {
        (session = sessions[i]).runQueries(self.options.teardown, function(err) {
            ...
            session._end(function() { --nleft <= 0 && callback && callback(errors[0], errors) })
        })
...
function Session( ) {
    ...
    this.sessionDb = null
    this.connect = this.end = this._getRawDb = null
    this.notifyBusy = this.notifyIdle = null
}
...
Session.prototype.runQueries = function runQueries( queries, callback ) {
    ...
    }, function(err) { callback(err, { durationMs: utils.microtime() - t1, conn: self }) })
}
...
Session.prototype._readResult = function _readResult(query, nextSeqId, t1, callback) {
    ...
    this.isReading = true; this.notifyBusy()

    var self = this
    var info = { duration_ms: 0, columnNames: [], conn: self }
    function _done( err, res ) {
        ...
        else self.isReading = false; self.notifyIdle()
...
Session.prototype._end = function _end( callback ) {
...
function formatValue(arg) {
    var t = typeof arg
    return (t === 'number') ? arg : (t === 'string') ? "'" + addslashes(arg) + "'"
        : (t === 'boolean') ? Number(arg) : (arg === null) ? 'NULL'
        : (arg instanceof Date) ? "'" + arg.toISOString() + "'"
        : (Array.isArray(arg)) ? arg.map(formatValue).join(', ')
