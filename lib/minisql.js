...
module.exports = {
    ...
    createConnection: function(options) { return new Db(options).getConnection() },
}
...
function Db( options ) {
    ...
    for (var i = 0; i < (this.options.sessions || 1); i++) {
        this.sessions.push(new Session(this.options))
    }
}
Db.prototype.connect = function connetc(callback) {
...
Db.prototype.getConnection = function getConnection( ) {
    // simple round-robin
    return this.sessions[this.lastUsedSession++ % this.sessions.length]
}
