...
function Packman( ) {
    ...
    this.chunker = new Packeteer({      // data chunks -> mysql packets
        ...
        assemblePayload: function(chunks) { return Buffer.concat(chunks) },
    })
    this.packeter = new Packeteer({     // mysql packets -> mysql responses
        ...
        assemblePayload: function(packets) {
            // given exactly bytesNeeded bytes spread across multiple chunks, assemble the packet
...
function Packeteer( options ) {
    ...
    this.assemblePayload = options.assemblePayload || function(bufs) { return Buffer.concat(bufs) }
    this.needed = 0
    ...
    this.get = function get( ) {
        ...
        var buf, have = 0, chunks = new Array()
        while (need > 0) {
            if (need > this.bufs[0].length) buf = this.bufs.shift()
            else { buf = this.bufs[0].slice(0, need); this.bufs[0] = this.bufs[0].slice(need) }
            chunks.push(buf); need -= buf.length; have += buf.length
        }
        return this.assemblePayload(chunks)
    }
}
