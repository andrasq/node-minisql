...
Db.prototype.ping = function ping( callback ) {
    return this.getConnection().ping(callback)
}
Db.prototype.runQueries = function runQueries( sqls, callback ) {
    return this.getConnection().runQueries(sqls, callback)
}
...
Db.prototype._select = function _select( sql, params, callback ) {
    return this.getConnection()._select(sql, params, callback)
}
Db.prototype.end = function end( callback ) {
    var self = this, nleft = this.sessions.length, errors = []
    for (var i=0; i<this.sessions.length; i++) this.sessions[i].end(function(err) {
        nleft -= 1; if (err) { err.conn = self.sessions[i].id; errors.push(err) }
        if (nleft === 0 && callback) callback(errors.length ? errors : null)
...
Session.prototype.connect = function connect( callback ) {
    if (this.packman._socket) return callback(_makeError('session %d already connected', this.id))
    var self = this, creds = utils.extractTo({}, this.options, { user: 1, password: 1, host: 1, port: 1, database: 1 })
