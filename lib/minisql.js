...
Db.prototype.getConnection = function getConnection( ) {
    ...
    return conn.destroyed ? this._replaceConnection(this.lastSession) : conn
}
Db.prototype._replaceConnection = function _replaceConnection( ix ) {
    var conn = this.sessions.splice(ix, 1)
    console.error("mysqule: connection %d lost, removing from pool", conn.id)
    if (this.connections.length > 0) { this.lastSession -= 1; return this.getConnection() }

    // TODO: if this.options.reconnect...
    // FIXME: do something than nicer than throwing an error if out of connections
    throw _makeError("fatal error: connection %d closed, no connections left", conn.id)
}
