...
function Db( options ) {
    this.options = extractTo({}, options || {}, {
        ...
        // TODO: timeout: 1, reconnect: 1, setup: 1, teardown: 1,
    })
    this.packman = new Packman()
    this._queryInfo = null
    this._busyQ = false
    this._queryQueue = new Array()
}
...
Db.prototype.queryDone = function queryDone(callback, err, res) {
    this._busyQ = false
    if (this._queryQueue.length) {
        var self = this, nextCall = self._queryQueue.shift()
        process.nextTick(function() { self.query(nextCall[0], nextCall[1], nextCall[2]) })
    }
    callback(err, res)
}
Db.prototype.query = function query( query, params, callback ) {
    ...
    // seeing some interleaved response packets, so serialize queries for now
    if (this._busyQ) { this._queryQueue.push([query, params, callback]); return }
    this._busyQ = true
    ...
    // var t1b = microtime();
    this._queryInfo = null;
    query = params = null // remove refs to free sooner
    ...
    this.packman.getPacket(function(never, buf) {
        ...
        // var t1c = microtime();
        if (buf[4] === myHeaders.OK && isOkPacket(buf)) return self.queryDone(callback, null, decodeOkPacket(buf)) // success but no data
        if (buf[4] === myHeaders.EOF && isEofPacket(buf)) return self.queryDone(callback, _makeError('unexpected EOF query response'))
        if (buf[4] === myHeaders.ERROR && isErrorPacket(buf)) return self.queryDone(callback, _makeQueryError(buf, query))
        if (buf[4] === myHeaders.LOCAL && isLocalInfilePacket(buf)) {
            // FIXME: either implement, or signal server that response is not forthcoming
            return self.queryDone(callback, _makeError('LOCAL INFILE not handled'), decodeResponsePacket(buf)) }
        ...
        self._getPacketsEof(null, function(errPacket, columnDefs) {
            if (errPacket) return self.queryDone(callback, _makeQueryError(errPacket, query));
            ...
            self._getPacketsEof(decodeRow, function(errPacket, rowbufs) {
                ...
                return self.queryDone(callback, null, rows)
            })
        })
    })
}
