...
Db.prototype.connect = function connect( creds, callback ) {
    var self = this, dbCreds = utils.extractTo({}, this.options, { user: 1, password: 1, host: 1, port: 1, database: 1 })
    if (!callback && typeof creds === 'function') { callback = creds; creds = utils.extractTo({}, this.options, dbCreds) }
    ...
    this.packman.getPacket(function(never, buf) {
        ...
        self._getPacketsEof(null, function(errPacket, columnDefs) {
            ...
            function decodeRow(rowbuf) {
                rows.push(my.decodeRowValues(columnCount, rowbuf, columnDecoders))
                //rows.push(my.decodeRowHash(columnCount, rowbuf, columnDecoders, columnNames))
            }
