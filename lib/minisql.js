...
Session.prototype._readResult = function _readResult(query, seqId, t1, callback) {
    ...
    var self = this
    function _done( err, res, info ) {
        var nextResult = self.readWaitlist.shift()
        if (!nextResult) self.isReading = false
        // use setImmediate to allow caller to fetch queryInfo
        else utils.setImmediate(function() { self.isReading = false; self._readResult.apply(self, nextResult) })
    ...
    this.packman.getPacket(function(never, buf) {
        ...
        self._getPacketsEof(null, function(errPacket, columnDefs) {
            ...
            self._getPacketsEof(decodeRow, function(errPacket, rowbufs) {
                ...
                var info = { duration_ms: t2 - t1, columnNames: columnNames, conn: self }
                return _done(null, rows, info)
            })
        })
    })
}
