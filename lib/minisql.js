...
function Db( options ) {
    ...
    var self = this, sessions = this.sessions = new Array()
    this.lastSession = 0
...
Db.prototype._getConnection = function _getConnection( ) {
    ...
    var i = this.lastSession, sessions = this.sessions
    if (!sessions[i].isReading) return sessions[i]
    do {
        i = (i+1) % sessions.length
        if (!sessions[i].isReading) { this.lastSession = i; return sessions[i] }
    } while (i !== this.lastSession)
    return sessions[i]
    // if (conn.destroyed) ... FIXME: should remove/replace, right now queries just error out
}
