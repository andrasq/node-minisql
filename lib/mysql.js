...
function decodeBytesTo(dst, data, bytes, format, names) {
    for (var i = 0; i < format.length; i++) {
        ...
        case 'e': dst[name] = bytes.getStringN(data.length - bytes.tell()); break
        case 'S': dst[name] = bytes.getStringN(bytes.getLenenc()); break        // string db data
        case 'N': dst[name] = bytes.getNumberN(bytes.lenenc()); break           // number db data
        case 'B': dst[name] = bytes.getBinaryN(bytes.lenenc()); break           // binary db data
        }
    }
    return dst
}
