...
// if data[3] === 0 then is an auth handshake packet (only case when server sends us a seq_id of 0)
function myHeaders() {
return {
    OK:                         0x00,           // OK packet
    EOF:                        0xFE,           // EOF packet
...
// TODO: abstract Packman packet boundary detection, use these lookup functions:
// function packetLength(buf) { return buf[0] + (buf[1] << 8) + (buf[2] << 16) }
// function packetType(buf) { return buf[3] === 0 ? 'auth'
//    : buf[4] === 0xfe ? 'eof' : buf[4] === 0 ? 'ok' : buf[4] === 0xfb ? 'infile' : buf[4] === 0xff ? 'error'
//    : undefined }
...
function encodeHandshakeResponse( sequenceId, handshake, clientCaps, username, pw, dbname ) {
    ...
    for (var i=0; i<6; i++) abytes.putUint4(0); abytes.seek(abytes.tell() - 1)
...
function decodeResponsePacket( data ) {
    if (data[3] === 0) return decodeHandshakePacket(data)
    switch (data[4]) {
...
function decodeErrorPacket(data) {
    ...
    var packet = { _seqId: sequence_id, _type: 'ERROR', header: header, error_code: errcode }

    if (errcode === 0xffff) {
        packet._type = 'ERROR_PROGRESS'
        // NOTE: the MariaDB description includes a leading 1 byte 'num_fields' field
        return bbytes.decodeBytesTo(packet, '113S', ['stage', 'max_stage', 'progress', 'progress_info'])
    }
    else {
        packet.sql_state_marker = bbytes.getStringN(1)
        packet.sql_state = bbytes.getStringN(5)
        packet.error_message = bbytes.getStringN(data.length - bbytes.tell())
        return packet;
    }
}

// see eg https://mariadb.com/kb/en/connection/#initial-handshake-packet
// seq id is always 0 on auth handshake, but only on auth,
// since that is the only case when the remote is the initiator
function decodeHandshakePacket(data) {
    bbytes.open(data, 5)
    var packet = { _seqId: data[3], _type: 'HANDSHAKE_REQ', protocol_version: data[4] }

    bbytes.decodeBytesTo(packet, 'z4 8121 2218++',
        ['server_version', 'connection_id',
         'auth_plugin_data_part_1', 'filler_1', 'capability_flag_1', 'character_set',
         'status_flags', 'capability_flags_2', 'auth_plugin_data_len', 'filler_2'])
    /**
    var packet = {
        _seqId: data[3],
        _type: 'HANDSHAKE_REQ',
...
/** not used currently, was for debugging handshake responses
