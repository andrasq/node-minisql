...
Db.prototype.connect = function connect( creds, callback ) {
    ...
    this.packman.getPacket(function(never, buf) {
        ...
        self.packman.getPacket(function(never, buf) {
            ...
            // TODO: if (creds.options) foreach option: runcmd(composeSetOptions(...))
            callback();
        })
    })
    return this;
...
// TODO: return only documented, guaranteed properties eg lastInsertId and affectedRows
        ...
        if (buf[4] === myHeaders.LOCAL && isLocalInfilePacket(buf)) return callback(_makeError('LOCAL INFILE not handled'))
        ...
        self._getPacketsEof(null, function(errPacket, columnDefs) {
            ...
            for (var i=0; i<columnDefs.length; i++) {
                ...
                columnDecoders[i] = (type >= 0xf9 && type <= 0xfc) ? bbytes.getBinaryN
                    : (type >= 0 && type <= 5 || type === 8 || type === 9 || type === 0x10 || type === 0xf6) ? bbytes.getNumberN
                    : bbytes.getStringN
                // TODO: see if worth compiling a typelist-specific decoder function
            }
            var rows = new Array(), hashes = new Array()
            function decodeRow(rowbuf) {
                // TODO: optionally decode into a hash, no speed penalty but issue with duplicate col names
                var row = decodeRowValues(columnCount, rowbuf, columnDecoders); rows.push(row)
            }
...
function decodeHandshakePacket(data) {
    ...
    if (true /* flags & myCaps.CLIENT_SECURE_CONNECTION *//* which we always set */) {
...
// FIXME: send as hex, insert as UNHEX(?)
    ...
    function writeString(buf, pos, str) {
        ...
        if (Buffer.isBuffer(buf) && buf.length >= pos + Buffer.byteLength(str)) return buf.write(str, pos)
    ...
    function readString(buf, fm, to) {
        ...
        return (to - fm <= 40) ? utf8.utf8_decode(buf, fm, to)
            : Buffer.isBuffer(buf) ? buf.toString('utf8', fm, to) : utf8.utf8_decode(buf, fm, to)
    }
}
