...
function Db( options ) {
    this.options = extractTo({}, options || {}, {
        user: 1, password: 1, host: 1, port: 1, database: 1, // timeout: 1, reconnect: 1, setup: 1, teardown: 1,
    })
    // TODO: keep array of connections, use first available; Mutex to acquire/release, w parametric isFree()
...
Db.prototype.connect = function connect( creds, callback ) {
    ...
    this.packman.getPacket(function(never, buf) {
        ...
        if (packet.protocol_version !== 10) return callback(_makeError('bad auth prototocol v %d', packet.protocol_version))
        ...
        self.packman.getPacket(function(never, buf) {
            if (!isOkPacket(buf)) return callback(_makeError('auth expected OK packet'));
            if (buf[3] !== 2) return callback(_makeError('wrong sequence_id %d in auth OK packet', buf[3]))
            // TODO: if (creds.setup) runCommands(creds.setup)
            callback();
        })
    })
    return this;
    ...
    this.packman.getPacket(function(never, buf) {
        ...
        self._getPacketsEof(null, function(errPacket, columnDefs) {
            ...
            self._getPacketsEof(decodeRow, function(errPacket, rowbufs) {
                if (errPacket) return callback(_makeQueryError(errPacket, query));
                var t2 = microtime()
                // typical 17m timing info: compose 15ms, query 140ms, decode 17ms (but faster to omit)
                // self._queryInfo = { duration_ms: t2 - t1, compose_ms: t1b - t1, query_ms: t1c - t1b, decode_ms: t2 - t1c, columns: columnDefs }
                self._queryInfo = { duration_ms: t2 - t1, columns: columnDefs }
                return callback(null, rows)
            })
        })
    })
}
Db.prototype.select = function select( query, params, callback ) {
    if (!callback) { callback = params; params = null }
    var self = this
    this.query(query, params, function(err, rows) {
        if (err) return callback(err)
        var columns = self.queryInfo().columns, names = new Array()
        for (var i=0; i<columns.length; i++) names.push(columns[i].name)
...
function decodeResponsePacket( data ) {
    ...
    // case myHeaders.EOF:  // using isEofPacket without decoding
    //     isEofPacket(data) decodeEofPacket(data) : decodeTextResultsetPacket(data)
    // default: return decodeTextResultsetPacket(data) // using getColumnCount instead
    }
}
