...
Packman.prototype._getResponse = function _getResponse( ) {
    ...
    // special-case single-packet responses, else locate final response message
...
Packman.prototype._makeBuffer = function _makeBuffer( a ) {
    // ensure that the data is in a buffer
...
// TODO: separate query() and execute() calls, for better typing of the expected results
        if (buf[4] === myHeaders.OK && isOkPacket(buf)) return callback(null, decodeOkPacket(buf))
        ...
        self._getPacketsEof(null, function(err, columnDefs) {
            ...
            if (columnDefs.length !== columnCount) {
                return callback(_makeError('bad column defs: expected %d, got %d', columnCount, columnDefs.length))
            }
...
function _makeError( props, fmt ) {
    ...
    if (typeof args[0] === 'object') for (var k in args[0]) err[k] = args[0][k]
    return err
}
