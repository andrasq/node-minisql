...
/**
// adapted from qibl (from miniq, actually)
function interpolate( template, patt, argv ) {
    var s = '', pos, lastPos = 0, argix = 0, arg;
    while ((pos = template.indexOf(patt, lastPos)) >= 0 && argix < argv.length) {
        s += template.slice(lastPos, pos) + formatValue(arg[argix++]);;
    ...
    function formatValue(arg) {
        return (typeof arg === 'number') ? arg : Buffer.isBuffer(arg) ? 'UNHEX("' + arg.toString('hex') + '")' : "'" + addslashes(arg) + "'";
        // TODO: : (Array.isArray(arg)) ? arg.map(function(e) { return formatValue(e) }).join(', ')
    }
}
// from qibl
