...
Db.prototype.connect = function connect( creds, callback ) {
    ...
    this.packman.getPacket(function(never, buf) {
        ...
        if (packet.protocol_version !== 10) return callback(_makeError('unsupport connection prototocol v %d', packet.protocol_version))
...
// TODO: separate query() and execute() calls, for better typing of the expected results
        ...
        self._getPacketsEof(null, function(errPacket, columnDefs) {
            ...
            for (var i=0; i<columnDefs.length; i++) {
                ...
                columnDecoders[i] = (type >= 0 && type <= 5 || type === 8 || type === 9 || type === 0x10 || type === 0xf6) ? bbytes.getNumberN
...
var myCaps = {
    ...
    CLIENT_COMPRESS:            0x00000020,     // supports compression zlib inflate/deflate ([3b compr DTA len][1b seq][[3b uncomp len] DTA])
    ...
    CLIENT_PS_MULTI_RESULTS:    0x00040000,     // allow ;-separated commands (NOTE: only first result is returned? or dequeue how many?)
...
// FIXME: send as hex, insert as UNHEX(?)
    ...
    function readString(buf, fm, to) {
        ...
        return (to - fm <= 40) ? utf8.utf8_decode(buf, fm, to) : Buffer.isBuffer(buf) ? buf.toString('utf8', fm, to) : utf8.utf8_decode(buf, fm, to)
    }
}
