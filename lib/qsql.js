...
Packman.prototype._getResponse = function _getResponse( ) {
    ...
    for (var i=1; i<packets.length; i++) {
        ...
        if (id1 + 1 !== id2) throw _makeError('packets %d, %d out of order: %d, %d', i-1, i, id1, id2)
    }
...
Packman.prototype.getPacket = function getPacket( callback ) {
    ...
    // FIXME: getPacket does not return errors, so how to indicate this.error ??
    // ... : this.error ? callback(this.error, fromBuf([]))
...
Db.prototype.connect = function connect( creds, callback ) {
    ...
    this.packman.getPacket(function(never, buf) {
        ...
        if (packet.protocol_version !== 10) return callback(_makeError('bad prototocol v %d', packet.protocol_version))
        ...
        self.packman.getPacket(function(never, buf) {
            ...
            // TODO: if (Array.isArray(creds.before)) run the commands on the newly opened connection to initialize it
            callback();
        })
    })
    return this;
...
Db.prototype.ping = function ping( callback ) {
    if (this.packman.error) return callback(_makeConnectionError(this.packman.error, 'COM_PING'))
    this.packman.sendPacket(composePing(), 0)
    this.packman.getPacket(function(never, buf) {
        // TODO: should not return raw responses, should return a supported format
        callback(null, decodeResponsePacket(buf))
    })
}
Db.prototype.quit = function quit( callback ) {
    // TODO: if (Array.isArray(this.options.after)) run the commands on the connection being closed to finalize it
    var err, self = this;
    if (this.packman.error) return callback(_makeConnectionError(this.packman.error, 'COM_QUIT'))
    this.packman.sendPacket(composeQuit(), 0)
/**
    this.packman.getPacket(function(never, buf) {
        // quit succeeds silently, calls back only on error
        err = _makeQueryError(buf, 'COM_QUIT')
    })
**/
    setTimeout(function() { self.packman.end(function() { callback && callback(err) }) }, 5);
}
Db.prototype.queryInfo = function queryInfo( ) {
    return this._queryInfo;
}
Db.prototype.query = function query( query, args, callback ) {
    var self = this;
    var t1 = microtime();
    if (!callback && typeof args === 'function') { callback = args; args = null }
    if (Array.isArray(args)) query = vinterpolate(query, '?', args)
    var queryCmd = composeQuery(query);
    var t1b = microtime();
    this._queryInfo = null;

    var packman = this.packman;
    if (packman.error) return callback(_makeConnectionError(packman.error, query))
    var seqId = packman.sendPacket(queryCmd, 0);
    packman.getPacket(function(never, buf) {
...
// TODO: return only documented, guaranteed properties eg lastInsertId and affectedRows
        ...
        if (buf[4] === myHeaders.ERROR && isErrorPacket(buf)) return callback(_makeQueryError(buf, query))
        if (buf[4] === myHeaders.LOCAL && isLocalInfilePacket(buf)) {
            return callback(_makeError('LOCAL INFILE not handled'), decodeResponsePacket(buf)) }

        // else the query succeeded and returned results, col count lenenc at offset 4
        var columnCount = getColumnCount(buf)
        self._getPacketsEof(null, function(errPacket, columnDefs) {
            if (errPacket) return callback(_makeQueryError(errPacket, query));
            ...
            for (var i=0; i<columnDefs.length; i++) {
                ...
                columnDefs[i] = { col: i, name: col.name, type: col.column_type, table: col.table }
            ...
            self._getPacketsEof(decodeRow, function(errPacket, rowbufs) {
                if (errPacket) return callback(_makeQueryError(errPacket, query));
                var t2 = microtime()
                self._queryInfo = { duration_ms: t2 - t1, compose_ms: t1b - t1, query_ms: t1c - t1b, decode_ms: t2 - t1c,
                    columns: columnDefs }
...
var myTypes = {
    ...
    // (type >= 0 && type <= 5 || type === 9) are numeric, parseFloat
...
function encodeHandshakeResponse( sequenceId, packet, username, pw, dbname ) {
    ...
    abytes.putUint4(0xffffff)   // max_packet_size: longest query client might send to server,
...
/** now using getColumnCount instead
...
function _makeConnectionError( err, query ) {
    return _makeError({ query: query }, 'connection error: %s', err.message)
}
