...
// TODO: separate query() and execute() calls, for better typing of the expected results
        ...
        self._getPacketsEof(null, function(errPacket, columnDefs) {
            ...
            for (var i=0; i<columnDefs.length; i++) {
                ...
                if (!type) return callback(_makeError('bad column defs: column %d has no type', i+1))
            ...
            function decodeRow(rowbuf) {
                ...
                //hashes.push(decodeRowHash(columnCount, rowbuf, columnDecoders, columnNames))
            }
            self._getPacketsEof(decodeRow, function(errPacket, rowbufs) {
                if (errPacket) return callback(_makeQueryError(errPacket, query));
                var t2 = microtime()
                var info = { duration_ms: t2 - t1, compose_ms: t1b - t1, query_ms: t1c - t1b, decode_ms: t2 - t1c, columns: columnDefs }
                return callback(null, rows, info)
            })
        })
    })
}
