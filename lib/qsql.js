...
Packman.prototype._getResponse = function _getResponse( ) {
    if (this.packets.length && this.packets[0].length < 4 + 0xffffff) return this.packets.shift()       // special case single-packet responses
    for (var end=0; end<this.packets.length && this.packets[end].length >= 4 + 0xffffff; end++) ;       // locate end of response
console.log("AR: _getResponse end", end, this.packets.length)
    if (end >= this.packets.length) return null

    // extract the response from multiple packets, `end` count starting at offset `0`
    var packets = this.packets.splice(0, end + 1)
console.log("AR: _getResponse: packets.length",packets.length)
    for (var i=1; i<packets.length; i++) {
//console.log("AR: queued packet len", packets[i].length)
        if (packets[i][3] !== packets[i-1][3] + 1) throw _makeError('packets %d,%d out of order: %d, %d', i-1, i, packets[i-1][3], packets[3])
        packets[i] = packets[i].slice(4)
    }
console.log("AR: removed packet, %d left", this.packets.length, this.packets[0] && this.packets[0].slice(0, 20))
...
Packman.prototype.sendPacket = function sendPacket( packet, seqId ) {
    ...
    if (packet.length > 4 + 0xfffffe) {
        while (packet.length - dataOffset >= 0xffffff) {
            ...
            this._socket.write(this._makeBuffer(packet.slice(dataOffset, dataOffset + 0xffffff)))
            dataOffset += 0xffffff
        }
        var tmpbuf = this._makeBuffer([0, 0, 0, 0])
        bbytes.writeUint3(tmpbuf, packet.length - dataOffset, 0)
        tmpbuf[3] = seqId
        this._socket.write(tmpbuf)
        this._socket.write(this._makeBuffer(packet.slice(dataOffset)))
    } else {
...
Db.prototype.query = function query( query, callback ) {
    ...
    this.packman.getPacket(function(err, buf) {
        ...
        case isOkPacket(buf):
            ...
            var packet = decodeOkPacket(buf)
// TODO: return only documented, guaranteed properties eg lastInsertId and affectedRows
// TODO: split query and execute, for better typing of the expected results
...
function isOkPacket(data) {
    return (data[4] === myHeaders.OK && data[0] === 7 && data[1] === 0 && data[2] === 0);
}
