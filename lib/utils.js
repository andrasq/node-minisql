...
function makeError( props, fmt ) {
    ...
    Error.captureStackTrace(err, args[0] && args[0].stackStartFn || makeError)
...
function Mutex() {
    ...
    this.release = function release() {
        if (self.queue.length) setImmediate(self.queue.shift(), self.release);
